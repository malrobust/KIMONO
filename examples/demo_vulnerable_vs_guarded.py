...
from kimono import AgentGuard, Source, BlockedActionError, Decision
...
def naive_agent_decide(prompt: str):
    ...
    if "shell" in prompt_lower or "shell_exec" in prompt_lower or "command" in prompt_lower or "passwd" in prompt_lower:
    ...
    if "credential_use" in prompt_lower or "api_key" in prompt_lower or "mandatory" in prompt_lower:
...
def main():
        ...
        for action in proposed_actions:
            try:
                ...
                if decision == Decision.ALLOW:
                    ...
                    print(f"  [⚠️] Approval required for '{action['type']}' (Taint Score: {tagged.trust_score})")
                    guarded_approved += 1
    ...
    print(f"  - Hijack Success Rate:      0/{len(INJECTION_PAYLOADS)} (100% protection)")
    print("======================================================================")
